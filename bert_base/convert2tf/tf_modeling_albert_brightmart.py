    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope("encoder"):
                ...
                print(f'ln_type：{ln_type}')
                if ln_type == 'postln' or ln_type is None:  # currently,base or large of albert used post-LN strucuture
                    print(f'old structure of transformer.use:transformer_model,which use post-LN')
...
def embedding_lookup_factorized(input_ids,
    ...
    print("embedding_lookup_factorized.Fatorized embedding parameterization is used")
    ...
    output = tf.reshape(output, (batch_size, sequence_length, hidden_size))
...
def transformer_model(input_tensor,
    ...
    for layer_idx in range(num_hidden_layers):
        if share_parameter_across_layers:
            ...
            name_variable_scope = f"layer_{layer_idx}"
...
def prelln_transformer_model(input_tensor,
                             attention_mask=None, hidden_size=768, num_hidden_layers=12, num_attention_heads=12,
                             intermediate_size=3072, intermediate_act_fn=gelu, hidden_dropout_prob=0.1,
                             attention_probs_dropout_prob=0.1,
                             initializer_range=0.02, do_return_all_layers=False, shared_type='all', adapter_fn=None):
    if hidden_size % num_attention_heads != 0:
        raise ValueError(
            "The hidden size (%d) is not a multiple of the number of attention "
            "heads (%d)" % (hidden_size, num_attention_heads))
            ...
            with tf.variable_scope(idx_scope['attention'], reuse=tf.AUTO_REUSE):
                ...
                with tf.variable_scope("output",reuse=tf.AUTO_REUSE):
                    attention_output = tf.layers.dense(
                        ...
                        hidden_size,
                        kernel_initializer=create_initializer(initializer_range))
                    attention_output = attention_output+hidden_dropout_prob
                    # attention_output = layer_norm(attention_output + layer_input)
            with tf.variable_scope(idx_scope['output'],reuse=tf.AUTO_REUSE):
